...
# Bun from bun.sh, ahead of the pacman package: the Arch repo lags the
# version the CI and the Dockerfile pin (package.json "packageManager").
fish_add_path --prepend $HOME/.bun/bin

# cachyos-config.fish points fish_greeting at fastfetch. Overridden here (after
# the source above) because it runs on every new shell — 72ms and a screenful of
# output per terminal, for information that doesn't change.
